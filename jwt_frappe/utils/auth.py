...
def get_token(user, pwd, expires_in=60, expire_on=None, device=None):
    ...
    token = get_bearer_token(user=user, jwt_access_expiry_time=_expires_in , jwt_refresh_expiry_time=60)
...
def get_bearer_token(user, jwt_access_expiry_time=60 , jwt_refresh_expiry_time=60):
    import hashlib
    import jwt
    import frappe.oauth
    ...
    token = frappe._dict(
        {
            ...
            "expires_in": jwt_access_expiry_time or 60,  # Default to 60 seconds
            "token_type": "Bearer",
    ...
    jwt_refresh_token = generate_jwt_token(user=user, expires_in=jwt_refresh_expiry_time)
    bearer_token = frappe.new_doc("OAuth Bearer Token")
    bearer_token.client = client.name
    bearer_token.jwt_access_token = jwt_access_token
    bearer_token.jwt_refresh_token = jwt_refresh_token
...
def get_jwt_token():
    ...
    return get_bearer_token(
        user=frappe.session.user, jwt_access_expiry_time=60, jwt_refresh_expiry_time=60
    )["access_token"]
